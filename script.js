...
async function initialiseKT13() {
  ...
  // Dengarkan pengesahan markah rasmi daripada Firebase secara masa nyata.
  try {
    const { C01FirebaseStorage } = await import("../../firebase-storage.js?v=20260804-kt13");

    C01FirebaseStorage.listenToStudentAssessment(
      13,
      assessment => {
      ...
      error => {
        console.error("Listener Firebase KT13 gagal:", error);
      }
    );
  } catch (error) {
    console.error("Firebase KT13 tidak dapat dimuatkan:", error);
  }
}
...
async function submitKT13() {
  ...
  // Paparkan keputusan tempatan dahulu supaya pelatih tidak perlu menunggu rangkaian.
  if (passed) {
    showPendingResult(profile.pendingAssessments[13]);
  } else {
  ...
  // Hantar keputusan yang sama ke Firebase.
  try {
    if (typeof submitButton !== "undefined" && submitButton) {
      submitButton.disabled = true;
      submitButton.textContent = "SEDANG HANTAR KE FIREBASE...";
    }
    ...
    if (typeof submitButton !== "undefined" && submitButton) {
      if (passed) {
        ...
        submitButton.textContent = "HANTAR JAWAPAN";
      }
    }
  } catch (error) {
    console.error("Markah KT13 gagal dihantar ke Firebase:", error);

    if (typeof submitButton !== "undefined" && submitButton) {
      submitButton.disabled = false;
      submitButton.textContent = "HANTAR SEMULA";
    }

    alert(
      "Markah telah disimpan pada peranti, tetapi belum berjaya dihantar ke Firebase. " +
      "Semak sambungan Internet dan cuba hantar semula."
    );
  }
}
...
function showOfficialResult(official) {
  ...
  const assessor = official.assessor || "Pegawai Penilai";
  const verifiedDate = official.verifiedAt
    ? new Date(
        official.verifiedAt?.toDate
          ? official.verifiedAt.toDate()
          : official.verifiedAt
  ...
  resultPanel.innerHTML = `
    <div class="result-card official">
      ...
      <p><strong>Pegawai Penilai:</strong> ${assessor}</p>
      <p><strong>Tarikh Pengesahan:</strong> ${verifiedDate}</p>
    </div>
  `;
}