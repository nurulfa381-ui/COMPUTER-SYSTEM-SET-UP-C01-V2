...
const missions = [
  ...
  },
  {
    ...
    notesBm: [
      "Driver ialah perisian pemacu yang membolehkan sistem operasi berkomunikasi dengan perkakasan seperti motherboard chipset, network adapter, graphic adapter, audio device, storage controller, printer dan periferal lain.",
      "Selepas pemasangan sistem operasi, juruteknik perlu membuka Device Manager untuk mengenal pasti peranti yang belum lengkap driver. Tanda amaran seperti Unknown Device, Other devices atau simbol kuning menunjukkan driver perlu dipasang atau disemak.",
      "Chipset driver biasanya dipasang dahulu kerana ia membantu sistem operasi mengenal pasti fungsi asas motherboard seperti controller, USB, PCIe, power management dan komunikasi antara komponen.",
      "Network/LAN/Wi-Fi driver sangat penting kerana tanpa driver rangkaian, komputer mungkin tidak boleh akses internet, domain, printer rangkaian atau kemas kini sistem.",
      "Graphic/display driver perlu dipasang supaya resolusi paparan, multi-monitor, acceleration grafik dan kestabilan display berfungsi dengan baik. Driver generik Windows mungkin tidak cukup untuk prestasi sebenar.",
      "Audio driver membolehkan speaker, microphone dan audio jack berfungsi dengan betul, terutama jika komputer digunakan untuk mesyuarat dalam talian atau pembelajaran multimedia.",
      "Driver hendaklah diambil daripada sumber rasmi seperti laman web pengeluar motherboard, laptop, GPU, printer atau Windows Update yang dipercayai. Elakkan laman pihak ketiga yang tidak sah kerana risiko malware, driver salah atau software tambahan.",
      "Juruteknik perlu memastikan driver sepadan dengan model perkakasan, versi sistem operasi, edisi 32-bit/64-bit dan nombor model yang tepat.",
      "Sebelum memasang driver, semak job request dan spesifikasi komputer. Komputer desktop custom mungkin perlu driver daripada pengeluar motherboard, manakala laptop biasanya menggunakan support page berdasarkan model penuh laptop.",
      "Selepas driver penting dipasang, komputer perlu restart jika diminta supaya service, controller dan konfigurasi driver dimuatkan dengan betul.",
      "Jika driver menyebabkan masalah seperti blue screen, tiada bunyi, paparan berkedip atau network gagal, juruteknik boleh gunakan Roll Back Driver, uninstall driver, atau pasang versi driver yang lebih sesuai.",
      "Pengesahan selepas pemasangan driver mesti dibuat: Device Manager tiada simbol amaran, network berfungsi, bunyi diuji, resolusi paparan sesuai, storage/controller stabil dan periferal berkaitan boleh digunakan.",
      "Versi driver, sumber driver, tarikh pemasangan, keputusan ujian dan isu yang diselesaikan perlu direkod dalam laporan kerja supaya mudah disemak semula oleh guru, penyelia atau juruteknik lain."
    ],
    notesEn: [
      "A driver is software that allows the operating system to communicate with hardware such as motherboard chipset, network adapter, graphics adapter, audio device, storage controller, printer and other peripherals.",
      "After operating system installation, the technician must open Device Manager to identify devices with incomplete drivers. Warning signs such as Unknown Device, Other devices or yellow symbols indicate drivers must be installed or checked.",
      "The chipset driver is usually installed first because it helps the operating system identify motherboard functions such as controllers, USB, PCIe, power management and component communication.",
      "Network/LAN/Wi-Fi driver is important because without network drivers, the computer may not access internet, domain, network printer or system updates.",
      "Graphics/display driver should be installed so display resolution, multi-monitor, graphics acceleration and display stability work properly. A generic Windows driver may not provide full performance.",
      "Audio driver enables speaker, microphone and audio jack functions, especially when the computer is used for online meetings or multimedia learning.",
      "Drivers should come from official sources such as motherboard, laptop, GPU, printer manufacturer websites or trusted Windows Update. Avoid unofficial third-party sites because of malware, wrong drivers or bundled software risk.",
      "The technician must ensure the driver matches the hardware model, operating system version, 32-bit/64-bit edition and exact model number.",
      "Before installing drivers, check the job request and computer specification. A custom desktop may need motherboard manufacturer drivers, while a laptop usually uses the support page for the full laptop model.",
      "After important drivers are installed, restart the computer when requested so services, controllers and driver configurations load correctly.",
      "If a driver causes problems such as blue screen, no sound, flickering display or network failure, the technician can use Roll Back Driver, uninstall the driver, or install a more suitable driver version.",
      "Post-installation verification must be performed: no Device Manager warning symbols, network works, audio is tested, display resolution is suitable, storage/controller is stable and related peripherals can be used.",
      "Driver version, driver source, installation date, test result and resolved issues should be recorded in the work report so teachers, supervisors or other technicians can review the work."
    ],
    activityBm: "Semak Device Manager, pilih driver yang betul mengikut peranti, pasang mengikut urutan selamat, restart jika perlu dan sahkan semua peranti berfungsi.",
    activityEn: "Check Device Manager, choose the correct driver for each device, install in a safe sequence, restart when required and verify all devices work."
  },
  {
    id: 10,
    code: "KP10",
...
const kp09Quiz = {
  bm: [
    [
      "Apakah fungsi utama driver dalam sistem komputer?",
      ["Menukar warna casing", "Membolehkan OS berkomunikasi dengan perkakasan", "Memadam semua fail pengguna", "Menggantikan PSU"],
      1
    ],
    [
      "Apakah tempat utama untuk semak driver yang belum lengkap selepas OS dipasang?",
      ["Recycle Bin", "Device Manager", "Wallpaper settings", "Calculator"],
      1
    ],
    [
      "Driver manakah biasanya dipasang dahulu jika disediakan oleh pengeluar?",
      ["Printer driver", "Game controller driver", "Chipset driver", "Font driver"],
      2
    ],
    [
      "Jika komputer tidak boleh akses internet selepas OS siap, driver yang perlu disemak ialah",
      ["Network/LAN/Wi-Fi driver", "Audio driver sahaja", "Wallpaper driver", "Keyboard theme"],
      0
    ],
    [
      "Mengapa driver perlu dimuat turun daripada sumber rasmi?",
      ["Supaya boleh pasang software rawak", "Untuk elak driver salah, malware dan fail tidak dipercayai", "Supaya komputer tidak perlu diuji", "Untuk tukar nama pengguna"],
      1
    ],
    [
      "Jika paparan resolusi rendah dan tidak stabil, driver yang paling berkaitan ialah",
      ["Audio driver", "Printer driver", "Graphic/display driver", "Mouse pad driver"],
      2
    ],
    [
      "Apakah tindakan selepas memasang driver penting jika sistem meminta restart?",
      ["Abaikan restart selama-lamanya", "Restart komputer dan semak semula fungsi peranti", "Format semula storage", "Padam Device Manager"],
      1
    ],
    [
      "Jika driver baru menyebabkan masalah paparan berkedip, tindakan sesuai ialah",
      ["Roll Back Driver atau pasang versi lebih stabil", "Tukar warna tema", "Cabut semua RAM", "Padam laporan"],
      0
    ],
    [
      "Pengesahan driver dianggap baik apabila",
      ["Device Manager tiada simbol amaran dan fungsi peranti diuji", "Wallpaper sudah cantik", "Nama folder ditukar", "Semua fail dimampatkan"],
      0
    ],
    [
      "Maklumat driver yang wajar direkod dalam laporan ialah",
      ["Versi driver, sumber, tarikh pemasangan dan keputusan ujian", "Muzik latar", "Nama kawan", "Warna meja"],
      0
    ]
  ],
  en: [
    [
      "What is the main function of a driver in a computer system?",
      ["Change casing colour", "Allow the OS to communicate with hardware", "Delete all user files", "Replace the PSU"],
      1
    ],
    [
      "Where should missing drivers be checked after OS installation?",
      ["Recycle Bin", "Device Manager", "Wallpaper settings", "Calculator"],
      1
    ],
    [
      "Which driver is usually installed first when provided by the manufacturer?",
      ["Printer driver", "Game controller driver", "Chipset driver", "Font driver"],
      2
    ],
    [
      "If the computer cannot access internet after OS setup, which driver should be checked?",
      ["Network/LAN/Wi-Fi driver", "Audio driver only", "Wallpaper driver", "Keyboard theme"],
      0
    ],
    [
      "Why should drivers be downloaded from official sources?",
      ["To install random software", "To avoid wrong drivers, malware and untrusted files", "So the computer does not need testing", "To change username"],
      1
    ],
    [
      "If the display resolution is low and unstable, which driver is most related?",
      ["Audio driver", "Printer driver", "Graphics/display driver", "Mouse pad driver"],
      2
    ],
    [
      "What should be done after installing an important driver if the system asks for restart?",
      ["Ignore restart forever", "Restart the computer and verify the device function", "Reformat storage", "Delete Device Manager"],
      1
    ],
    [
      "If a new driver causes screen flickering, what is a suitable action?",
      ["Roll Back Driver or install a more stable version", "Change theme colour", "Remove all RAM", "Delete the report"],
      0
    ],
    [
      "Driver verification is good when",
      ["Device Manager has no warning symbols and device functions are tested", "Wallpaper looks nice", "Folder name is changed", "All files are zipped"],
      0
    ],
    [
      "Which driver information should be recorded in the report?",
      ["Driver version, source, installation date and test result", "Background music", "Friend's name", "Table colour"],
      0
    ]
  ]
};
...
function renderMission(id) {
  ...
  renderChrome(`
    <section class="mission-page">
      ...
      ${mission.id === 9 ? renderKp09Premium() : ""}
...
function renderSimulation(mission) {
  ...
  if (mission.id === 9) {
    ...
    const devices = bm
      ? [
          ["Unknown PCI Device", "chipset", "Pasang chipset driver dahulu supaya controller motherboard dikenal pasti dengan betul."],
          ["Ethernet Controller", "network", "Pasang driver LAN/network supaya komputer boleh akses internet dan rangkaian."],
          ["Video Controller", "graphics", "Pasang graphic/display driver supaya resolusi dan prestasi paparan stabil."],
          ["Multimedia Audio Controller", "audio", "Pasang audio driver supaya speaker, mic dan audio jack berfungsi."],
          ["Unknown Printer", "printer", "Pasang driver printer rasmi mengikut model printer dan sambungan yang digunakan."],
          ["Storage Controller Warning", "storage", "Semak storage/controller driver jika storage tidak stabil atau prestasi drive tidak normal."]
        ]
      : [
          ["Unknown PCI Device", "chipset", "Install chipset driver first so motherboard controllers are identified properly."],
          ["Ethernet Controller", "network", "Install LAN/network driver so the computer can access internet and network."],
          ["Video Controller", "graphics", "Install graphics/display driver so resolution and display performance are stable."],
          ["Multimedia Audio Controller", "audio", "Install audio driver so speaker, mic and audio jack work."],
          ["Unknown Printer", "printer", "Install the official printer driver according to printer model and connection type."],
          ["Storage Controller Warning", "storage", "Check storage/controller driver if storage is unstable or drive performance is abnormal."]
        ];
    const actions = bm
      ? [
          ["chipset", "Chipset driver rasmi"],
          ["network", "LAN / Wi-Fi driver"],
          ["graphics", "Graphic / display driver"],
          ["audio", "Audio driver"],
          ["printer", "Printer driver ikut model"],
          ["storage", "Storage controller driver"]
        ]
      : [
          ["chipset", "Official chipset driver"],
          ["network", "LAN / Wi-Fi driver"],
          ["graphics", "Graphics / display driver"],
          ["audio", "Audio driver"],
          ["printer", "Printer model driver"],
          ["storage", "Storage controller driver"]
        ];

    return `
      <div class="main-sim kp09-main-sim">
        <div class="sim-badge">${bm ? "KP09 V17 - SIMULASI DEVICE MANAGER DAN DRIVER" : "KP09 V17 - DEVICE MANAGER AND DRIVER SIMULATION"}</div>
        <div class="sim-instruction">
          ${bm ? "Klik peranti bermasalah dalam Device Manager, kemudian pilih driver atau tindakan yang betul." : "Click the problem device in Device Manager, then choose the correct driver or action."}
        </div>
        <div class="kp09-driver-board">
          <div class="device-manager-window">
            <div class="window-title">Device Manager</div>
            <div class="device-tree">
              <span class="device-ok">Display adapters - Generic Display</span>
              <span class="device-warn">Other devices - Unknown PCI Device</span>
              <span class="device-warn">Network adapters - Ethernet Controller</span>
              <span class="device-warn">Sound - Multimedia Audio Controller</span>
              <span class="device-ok">Disk drives - NVMe SSD</span>
            </div>
          </div>
        ...
        <div class="main-sim-grid">
          <div class="main-sim-panel">
            <h3>${bm ? "1. Pilih peranti" : "1. Choose device"}</h3>
            <div class="sim-choice-list">
              ${devices.map(([title, answer, info]) => `
                <button type="button" class="sim-choice" data-kp09-device="${title}" data-kp09-answer="${answer}" data-kp09-info="${info}">
                  <strong>${title}</strong>
                  <small>${bm ? "Klik untuk tentukan driver" : "Click to decide driver"}</small>
                </button>
              `).join("")}
            </div>
          </div>
          <div class="main-sim-panel">
            <h3>${bm ? "2. Pilih tindakan driver" : "2. Choose driver action"}</h3>
            <div class="sim-category-grid">
              ${actions.map(([value, label], index) => `<button type="button" class="btn" data-kp09-driver="${value}" style="order:${(index * 2 + 1) % actions.length}">${label}</button>`).join("")}
            </div>
          </div>
          <div class="main-sim-panel">
            <h3>${bm ? "3. Maklum balas" : "3. Feedback"}</h3>
            <div id="kp09MainResult" class="hotspot-result">${bm ? "Belum mula. Pilih satu peranti bermasalah." : "Not started. Choose one problem device."}</div>
            <div id="kp09MainGuide" class="procedure-card muted">${bm ? "Sistem akan beri panduan sumber driver, restart dan pengesahan selepas jawapan betul." : "The system will show driver source, restart and verification guidance after a correct answer."}</div>
            <div id="kp09MainScore" class="score-pill">${bm ? "Skor latihan: 0/0" : "Drill score: 0/0"}</div>
          </div>
        </div>
      </div>
    `;
...
function renderKp09Premium() {
  const bm = state.lang === "bm";
  const labels = bm
    ? {
        video: "Animasi Ringkas: Dari Unknown Device ke Driver Lengkap",
        map: "Peta Urutan Pemasangan Driver",
        scenario: "Simulasi Ralat Driver",
        activities: "Aktiviti Pelajar",
        game: "Mini Game: Pilih Driver Yang Sesuai",
        gameHelp: "Klik tindakan yang paling sesuai untuk setiap simptom driver.",
        done: "Betul. Tindakan itu sesuai untuk masalah driver tersebut.",
        wrong: "Belum tepat. Semak jenis peranti dan sumber driver."
      }
    : {
        video: "Short Animation: From Unknown Device to Complete Driver",
        map: "Driver Installation Sequence Map",
        scenario: "Driver Error Simulation",
        activities: "Student Activities",
        game: "Mini Game: Choose the Suitable Driver",
        gameHelp: "Click the most suitable action for each driver symptom.",
        done: "Correct. That action suits the driver problem.",
        wrong: "Not yet. Check the device type and driver source."
      };

  const driverMap = bm
    ? [
        ["1. Semak Device Manager", "Kenal pasti Unknown Device, Other devices atau simbol amaran kuning."],
        ["2. Sahkan model", "Semak model motherboard/laptop/printer/GPU sebelum download driver."],
        ["3. Chipset dahulu", "Pasang chipset driver sebelum driver rangkaian, grafik dan audio jika disediakan."],
        ["4. Network driver", "Pastikan LAN/Wi-Fi berfungsi supaya update dan akses rangkaian boleh dibuat."],
        ["5. Graphic dan audio", "Pasang driver paparan dan bunyi untuk fungsi multimedia yang stabil."],
        ["6. Restart", "Restart selepas driver penting untuk memuatkan service dan konfigurasi."],
        ["7. Verify", "Pastikan tiada warning, resolusi betul, bunyi ada dan network boleh digunakan."],
        ["8. Rekod laporan", "Catat versi driver, sumber, tarikh dan keputusan ujian."]
      ]
    : [
        ["1. Check Device Manager", "Identify Unknown Device, Other devices or yellow warning symbols."],
        ["2. Confirm model", "Check motherboard/laptop/printer/GPU model before downloading drivers."],
        ["3. Chipset first", "Install chipset driver before network, graphics and audio when available."],
        ["4. Network driver", "Ensure LAN/Wi-Fi works so updates and network access are available."],
        ["5. Graphics and audio", "Install display and sound drivers for stable multimedia function."],
        ["6. Restart", "Restart after important drivers to load services and configuration."],
        ["7. Verify", "Ensure no warnings, correct resolution, audio works and network is usable."],
        ["8. Record report", "Record driver version, source, date and test result."]
      ];

  const scenarios = bm
    ? [
        ["Tiada internet selepas OS siap", "Semak Device Manager, pasang LAN/Wi-Fi driver rasmi, kemudian uji ping/browser."],
        ["Paparan besar dan resolusi rendah", "Pasang graphic/display driver rasmi mengikut model GPU atau laptop."],
        ["Bunyi tidak keluar", "Semak audio device, pasang audio driver dan uji speaker/microphone."],
        ["Driver baru menyebabkan masalah", "Gunakan Roll Back Driver, uninstall driver atau pasang versi yang lebih stabil."]
      ]
    : [
        ["No internet after OS setup", "Check Device Manager, install official LAN/Wi-Fi driver, then test ping/browser."],
        ["Large display and low resolution", "Install official graphics/display driver according to GPU or laptop model."],
        ["No sound output", "Check audio device, install audio driver and test speaker/microphone."],
        ["New driver causes problems", "Use Roll Back Driver, uninstall driver or install a more stable version."]
      ];

  const activities = bm
    ? [
        "Aktiviti 1: Pelajar baca paparan Device Manager dan tandakan peranti yang ada simbol amaran.",
        "Aktiviti 2: Pelajar susun urutan driver: chipset, network, graphic, audio dan periferal.",
        "Aktiviti 3: Pelajar padankan hardware ID/simptom dengan jenis driver yang sesuai.",
        "Aktiviti 4: Pelajar pilih sumber driver rasmi berdasarkan model motherboard/laptop/printer.",
        "Aktiviti 5: Pelajar lengkapkan checklist selepas driver: restart, network test, audio test, display resolution dan warning status.",
        "Aktiviti 6: Pelajar bincang risiko download driver daripada laman tidak rasmi.",
        "Aktiviti 7: Pelajar buat simulasi rollback driver apabila driver baru menyebabkan masalah.",
        "Aktiviti 8: Pelajar sediakan catatan laporan driver lengkap: versi, sumber, tarikh, keputusan ujian dan isu selesai."
      ]
    : [
        "Activity 1: Students read Device Manager and mark devices with warning symbols.",
        "Activity 2: Students arrange driver order: chipset, network, graphics, audio and peripherals.",
        "Activity 3: Students match hardware ID/symptom with the suitable driver type.",
        "Activity 4: Students choose official driver sources based on motherboard/laptop/printer model.",
        "Activity 5: Students complete post-driver checklist: restart, network test, audio test, display resolution and warning status.",
        "Activity 6: Students discuss risks of downloading drivers from unofficial sites.",
        "Activity 7: Students simulate driver rollback when a new driver causes problems.",
        "Activity 8: Students prepare complete driver report notes: version, source, date, test result and resolved issue."
      ];

  const gameItems = bm
    ? [
        ["Device Manager tunjuk Unknown PCI Device.", "chipset"],
        ["Komputer tidak boleh internet melalui LAN.", "network"],
        ["Resolusi paparan hanya 1024x768.", "graphics"],
        ["Speaker dan microphone tidak berfungsi.", "audio"],
        ["Printer dikesan tetapi tidak boleh print.", "printer"],
        ["Driver baru menyebabkan skrin berkedip.", "rollback"]
      ]
    : [
        ["Device Manager shows Unknown PCI Device.", "chipset"],
        ["Computer cannot access internet through LAN.", "network"],
        ["Display resolution is only 1024x768.", "graphics"],
        ["Speaker and microphone do not work.", "audio"],
        ["Printer is detected but cannot print.", "printer"],
        ["New driver causes screen flickering.", "rollback"]
      ];

  const choices = bm
    ? [
        ["chipset", "Pasang chipset driver"],
        ["network", "Pasang LAN/Wi-Fi driver"],
        ["graphics", "Pasang graphic driver"],
        ["audio", "Pasang audio driver"],
        ["printer", "Pasang printer driver rasmi"],
        ["rollback", "Roll back / tukar versi driver"]
      ]
    : [
        ["chipset", "Install chipset driver"],
        ["network", "Install LAN/Wi-Fi driver"],
        ["graphics", "Install graphics driver"],
        ["audio", "Install audio driver"],
        ["printer", "Install official printer driver"],
        ["rollback", "Roll back / change driver version"]
      ];
  ...
  return `
    <div class="kp01-grid">
      <div class="panel lesson-box">
        <h2>${labels.video}</h2>
        <div class="kp09-driver-animation">
          <div class="driver-card unknown">Unknown Device</div>
          <div class="driver-arrow">Driver Match</div>
          <div class="driver-card install">Installing</div>
          <div class="driver-arrow">Restart</div>
          <div class="driver-card ready">Ready</div>
        </div>
      </div>
      ...
      <div class="panel lesson-box wide">
        <h2>${labels.map}</h2>
        <div class="component-map-grid">
          ${driverMap.map(([title, detail]) => `<div><strong>${title}</strong><p>${detail}</p></div>`).join("")}
        </div>
      </div>
      ...
      <div class="panel lesson-box wide">
        ...
        <div class="game-list">
          ${gameItems.map(([item, answer], index) => `
            <div class="game-card" data-kp09-card="${index}">
              <strong>${item}</strong>
              <div class="game-options">
                ${orderedChoices(choices, index).map(([value, label]) => `<button class="btn" data-kp09-answer="${value}" data-correct="${answer}" data-card="${index}">${label}</button>`).join("")}
              </div>
            </div>
          `).join("")}
        </div>
        <div id="kp09GameResult" class="result"></div>
        <template id="kp09GameText" data-done="${labels.done}" data-wrong="${labels.wrong}"></template>
      </div>
    </div>
  `;
}

let activeKp09Device = null;
let kp09MainStats = { attempts: 0, correct: 0 };
let kp09CorrectDevices = new Set();

function selectKp09Device(button) {
  activeKp09Device = {
    title: button.dataset.kp09Device,
    answer: button.dataset.kp09Answer,
    info: button.dataset.kp09Info
  };

  document.querySelectorAll("[data-kp09-device]").forEach((item) => item.classList.remove("active"));
  document.querySelectorAll("[data-kp09-driver]").forEach((item) => item.classList.remove("selected", "correct", "incorrect"));
  button.classList.add("active");

  const result = document.getElementById("kp09MainResult");
  const guide = document.getElementById("kp09MainGuide");
  if (result) {
    result.classList.remove("correct", "incorrect");
    result.innerHTML = `<strong>${activeKp09Device.title}</strong><span>${state.lang === "bm" ? "Sekarang pilih driver atau tindakan yang sesuai." : "Now choose the suitable driver or action."}</span>`;
  }
  if (guide) {
    guide.classList.add("muted");
    guide.textContent = state.lang === "bm"
      ? "Pilih tindakan driver dahulu. Panduan pengesahan akan keluar selepas jawapan betul."
      : "Choose a driver action first. Verification guidance will appear after a correct answer.";
  }
}

function checkKp09Driver(button) {
  const result = document.getElementById("kp09MainResult");
  const guide = document.getElementById("kp09MainGuide");
  const score = document.getElementById("kp09MainScore");
  ...
  if (!activeKp09Device) {
    if (result) {
      result.classList.remove("correct", "incorrect");
      result.innerHTML = `<strong>${state.lang === "bm" ? "Pilih peranti dahulu" : "Choose a device first"}</strong><span>${state.lang === "bm" ? "Klik satu peranti bermasalah dalam Device Manager." : "Click one problem device in Device Manager."}</span>`;
    }
    return;
  }

  const correct = button.dataset.kp09Driver === activeKp09Device.answer;
  kp09MainStats.attempts += 1;
  if (correct) {
    kp09CorrectDevices.add(activeKp09Device.title);
    kp09MainStats.correct = kp09CorrectDevices.size;
  }
  ...
  if (result) {
    ...
    result.classList.toggle("incorrect", !correct);
    result.innerHTML = `
      <strong>${correct ? (state.lang === "bm" ? "Betul" : "Correct") : (state.lang === "bm" ? "Belum tepat" : "Not yet")}: ${activeKp09Device.title}</strong>
      <span>${correct ? activeKp09Device.info : (state.lang === "bm" ? "Cuba tindakan lain. Padankan jenis peranti dengan driver yang sesuai." : "Try another action. Match the device type with the suitable driver.")}</span>
    `;
  }
  if (guide) {
    guide.classList.toggle("muted", !correct);
    guide.innerHTML = correct
      ? `<strong>${state.lang === "bm" ? "Pengesahan selepas driver" : "Post-driver verification"}</strong><span>${state.lang === "bm" ? "Restart jika diminta, buka semula Device Manager, pastikan tiada simbol amaran dan uji fungsi peranti." : "Restart when requested, reopen Device Manager, ensure no warning symbols and test the device function."}</span>`
      : (state.lang === "bm" ? "Belum tepat. Semak dahulu sama ada masalah berkaitan chipset, network, paparan, audio, printer atau storage." : "Not yet. Check whether the issue relates to chipset, network, display, audio, printer or storage.");
  }
  if (score) {
    score.textContent = state.lang === "bm"
      ? `Skor latihan: ${kp09MainStats.correct}/${kp09MainStats.attempts}`
      : `Drill score: ${kp09MainStats.correct}/${kp09MainStats.attempts}`;
  }
}

function bindKp09Game() {
  activeKp09Device = null;
  kp09MainStats = { attempts: 0, correct: 0 };
  kp09CorrectDevices = new Set();

  document.querySelectorAll("[data-kp09-device]").forEach((button) => {
    button.addEventListener("click", () => selectKp09Device(button));
  });

  document.querySelectorAll("[data-kp09-driver]").forEach((button) => {
    button.addEventListener("click", () => checkKp09Driver(button));
  });

  document.querySelectorAll("[data-kp09-answer]").forEach((button) => {
    button.addEventListener("click", () => {
      const card = document.querySelector(`[data-kp09-card="${button.dataset.card}"]`);
      const result = document.getElementById("kp09GameResult");
      const gameText = document.getElementById("kp09GameText");
      const correct = button.dataset.kp09Answer === button.dataset.correct;

      card.querySelectorAll("[data-kp09-answer]").forEach((item) => item.classList.remove("selected"));
      button.classList.add("selected");
      card.classList.toggle("correct", correct);
      card.classList.toggle("incorrect", !correct);
      result.innerHTML = `<div class="${correct ? "success-note" : "error"}">${correct ? gameText.dataset.done : gameText.dataset.wrong}</div>`;
...
function buildQuiz(id) {
  ...
  if (id === 9) return varyQuizAnswerPositions(kp09Quiz[state.lang], id);
